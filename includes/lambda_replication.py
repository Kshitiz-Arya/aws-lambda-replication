...
def generateFunctionTemplate(fileToCopy, fileToSave, config):
    ...
    # Open the lambdaTemplate.json generic file
...
def readConfig():
    ...
    DBSourceTemplate = config["targetFunction"]
    DBtoGenerate = config["sourceFunction"]

    return config, DBSourceTemplate, DBtoGenerate

def deployFunctions(SourceTemplate, toGenerate, config):
    lambdaConfig = [ generateFunctionTemplate(SourceTemplate, entry + "_SourceFunction", config) for entry in toGenerate ]
    
    with open('config/lambdaConfig.yml', 'w') as outfile:
        print("Writing: %s to a JSON file in Inputs\n" % 'lambdaConfig.yml')
        outfile.write("lambdaConfig: " + str(lambdaConfig))
        # json.dump(lambdaConfig, outfile, indent=4, sort_keys=True)


# Removes a list of keys from the Lambda JSON
def cleanFunctionConfig(data):
    listOfRemovalKeys = ["CodeSha256", "FunctionArn", "LastModified", "RevisionId", "TracingConfig", "Version"]
